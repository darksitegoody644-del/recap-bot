...
async def download_video(url, output_dir):
    """Download video using pytubefix for YouTube, yt-dlp for others"""
    output_path = os.path.join(output_dir, "input_video.mp4")
    
    if "youtube.com" in url or "youtu.be" in url:
        # Use pytubefix for YouTube (no JS runtime needed)
        ...
        try:
            ...
            raise Exception(f"YouTube download failed: {str(e)[:200]}")
    else:
        # Use yt-dlp for TikTok and others
        cmd = [
            "yt-dlp",
            "-f", "best[ext=mp4]/best",
